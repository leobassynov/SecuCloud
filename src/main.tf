...
resource "azurerm_network_security_group" "ynov1-nsg" {
  ...
  # Allow incoming connection on port 80 for HTTP
  security_rule {
    name                       = "HTTP"
    priority                   = 1002
    direction                  = "Inbound"
...
backend "azurerm" {
    resource_group_name  = "tfstate-ynov-rg"
    storage_account_name = "tfstateynov73906lb" # MENS TON NOUVEAU NOM ICI
...
provider "azurerm" {
  ...
  subscription_id = "5d30161e-1be5-41fd-b530-30d4b7689f6f"
}
